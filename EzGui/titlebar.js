customElements.define("ahk-nav", class extends HTMLElement {
	connectedCallback() {
		const buttons = {
		...
		for (var key in buttons) {
			if (this.hasAttribute(key)) {
				buttonlist.push(`<span name=${key} onclick="gui.${key}()">${buttons[key]}</span>`)
			}
		}

		this.innerHTML = `
			<header>
				<span onmousedown="gui.captionMove()">${document.title}</span>
				<div class="buttons">
					${buttonlist.join(" ")}
				</div>
			</header>
